/*
  ROOT macro to produce DSTs for Veto Self Trigger Mode
  *****************For Self Trigger Mode************
  How to run it:
  g++ `root-config --cflags --libs` AwaySelfMacroDST.C -o AwaySelf
...
void AwaySelfMacroDST(TString run_number) {
	...
	string  od_path  = "/ds50/data/test_processing/lsv/v6/Calibration/";
	...
	TString outFileName;
	//	outFileName.Form("./FermiDSTAwayData/DST_Run%06d.root",run_id_list[0]);
	outFileName.Form("./FermiDSTSelfData/DST_Run%06d.root",run_id_list[0]);
...
#else  
  //Random Trigger Data
  const Double_t od_coinc_wind = 0.; //[ns] obtained from validation fits
#endif
	...
	Float_t  od_lsv_total_spe_charge          = -1;
	...
	*/
	...
	TArrayF         *od_lsv_cluster_charge;
	TArrayF         *od_lsv_cluster_start;
	TArrayF         *od_lsv_cluster_multiplicity;
	TArrayF         *od_lsv_cluster_height;
	TArrayF         *od_lsv_cluster_width;

	od_lsv_cluster_charge = 0;
	od_lsv_cluster_start = 0;
	od_lsv_cluster_multiplicity = 0;
	od_lsv_cluster_height = 0;
	od_lsv_cluster_width = 0;
	...
	od_chain->SetBranchAddress("lsv_total_spe_charge", &od_lsv_total_spe_charge);
	...
	od_chain->SetBranchAddress("lsv_cluster_width_ns", &od_lsv_cluster_width);
	...
	Int_t od_eventID       = -1;                                                                                         
        ...
        Double_t od_total_spe_charge  = -1.;                          
        ...
        DSTtree->Branch("od_total_spe_charge", &od_total_spe_charge,"od_total_spe_charge/D");                
        ...
        DSTtree->Branch("od_cluster_width", &od_cluster_width);            
	...
	for (Int_t n_od = n_od_begin; n_od < n_od_end; n_od++){
	  ...
	  if(!(n_od%50000)) std::cout<<"Processing Event "<<n_od<<", "
				       <<Int_t(100*n_od/n_od_end)<<"% Completed"<<std::endl;
#ifdef normal
	  ...
	  od_total_spe_charge = od_lsv_total_spe_charge+0.;
	  ...
	  for (Int_t n_clu = 0 ; n_clu < od_lsv_nclusters; n_clu++){
	    ...
	    */
	    ...
	    od_cluster_width.push_back(od_lsv_cluster_width->At(n_clu));
