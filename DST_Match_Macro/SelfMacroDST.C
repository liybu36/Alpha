/*
  ROOT macro to produce DSTs for Veto Self Trigger Mode and 
  match the OD with TPC
...
void SelfMacroDST(TString run_number) {
...
//struct VetoEvent;
	...
	TString outFileName;
	outFileName.Form("./DSTSelfData/DST_Run%06d.root",run_id_list[0]);
...
//	    ODEvent.at(od_counts).od_timestamp = od_timestamp_temp;
	    ODEvent.at(od_counts).od_timestamp = tmp_od_timestamp;
	    ODEvent.at(od_counts).od_tpc_timestamp_diff = DT;
