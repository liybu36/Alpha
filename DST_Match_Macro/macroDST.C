/*
  ROOT macro to produce DSTs for Veto Slave Mode
  How to run it:
  source darkart.sh
  .x macroDST.C+
*/
...
void macroDST() {
	...
	string  od_path  = "/ds50/data/test_processing/lsv/v6/Calibration/ODRun";

	std::vector<int> run_id_list;

	run_id_list.push_back(11148);

	TString outFileName;
	outFileName.Form("./FermiDSTSlaveData/DST_Run%06d.root",run_id_list[0]);
	//	outFileName.Form("./DSTBa133Data/DST_Run%06d.root",run_id_list[0]);
	  ...
	  Double_t DT =-100000;	
		...
		if ( !cut_gamma && !cut_neutron ) continue;
			...
			const Int_t n_od_end = (n_tpc + 10 > tpc_events ) ? tpc_events : (n_tpc + 10);
			     ...
			     od_timestamp     = od_pps_counter*1.e+6 + od_gps_fine_time_counter*20.e-3 - 54.*20.e-3; // [us]
			     od_total_spe_charge = od_lsv_total_spe_charge+0.;
			     
			     DT = abs(od_timestamp - tpc_timestamp);
			     ...
			     for (Int_t n_clu = 0 ; n_clu < od_nclusters; n_clu++){
