...
int main(int argc, char **argv){
  ...
  */
  ...
  TH1F* nv_gamma_first_r_gps_hist = new TH1F("nv_gamma_first_r_gps_hist","Veto Gamma First Alpha+gamma Time;Time [ns]",500,first_time,window2end);//delay_time);    
  ...
  TH1F* nv_neutron_first_r_gps_hist = new TH1F("nv_neutron_first_r_gps_hist","Veto Neutron First Alpha+neutron Time;Time [ns]",500,first_time,window2end);//delay_time);    
  ...
  TH1F* nv_neutron_window1_first_gps_hist = new TH1F("nv_neutron_window1_first_gps_hist","Veto Neutron Window1 First Time;Time [ns]",gamma_first_bins,window1start,window1end);
	...
	for(size_t i=0; i<ODEvent->size(); i++)
	  { 	  	 	  
	    ...
	    for(int ncluster=0; ncluster<ODEvent->at(i).od_nclusters; ncluster++)
	      {
		if(ODEvent->at(i).od_cluster_charge.at(ncluster)>0 && od_wt_charge<500) 
		  {
		    //*********************All OD cluster******************************************************************		    
	    ...
	    if(lsv_gamma_gps.size())
	    {
	      for(int ncluster=0; ncluster<od_nclusters; ncluster++)
		{
		  if(od_cluster_charge->at(ncluster)>0 && od_cluster_pass_multcut->at(ncluster)==1 && tpc_event_type==0 && od_wt_charge<500)
			...
			{ ++g;
			  if(g==1)
				...
				{ alpha_counts++;	nv_gamma_first_alpha_gps_hist->Fill(gps_gamma_diff_temp); }
				...
				nv_gamma_first_r_gps_hist->Fill(gps_gamma_diff_temp);
			      if(ODEvent->at(i).od_cluster_charge.at(ncluster)>pos3 && ODEvent->at(i).od_cluster_charge.at(ncluster)<pos2)
				neutron_counts++;
			    }
			}
	  ...
	  if(lsv_neutron_gps.size())
	    {
	      for(int ncluster=0; ncluster<od_nclusters; ncluster++)
		{
		  if(od_cluster_charge->at(ncluster)>0 && od_cluster_pass_multcut->at(ncluster)==1 && tpc_event_type==1 && od_wt_charge<500)
		    {
		      double gps_neutron_diff_temp =  od_cluster_dtprompt->at(ncluster)*1.e+3;
			  ...
			  if(n==1)
			    {
			      ...
			      if(ODEvent->at(i).od_cluster_charge.at(ncluster)>pos3 && ODEvent->at(i).od_cluster_charge.at(ncluster)<pos4)
				{ nalpha_counts++;	nv_neutron_first_alpha_gps_hist->Fill(gps_neutron_diff_temp);}
			      if(ODEvent->at(i).od_cluster_charge.at(ncluster)>pos1 && ODEvent->at(i).od_cluster_charge.at(ncluster)<pos2)
  ...
  double br5 = nv_neutron_first_small_ene_subtracted_hist->Integral(nv_neutron_first_small_ene_subtracted_hist->FindBin(pos3),nv_neutron_first_small_ene_subtracted_hist->FindBin(pos4));
  double br6 = nv_neutron_first_small_ene_subtracted_hist->Integral(nv_neutron_first_small_ene_subtracted_hist->FindBin(pos3),nv_neutron_first_small_ene_subtracted_hist->FindBin(pos2));
  cout<<"Neutron first "<<1.*br5/br6<<endl;
