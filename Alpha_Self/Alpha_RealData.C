...
int main(int argc, char **argv){
  ...
  odtree->SetBranchAddress("event_number", &event_number);
  odtree->SetBranchAddress("lsv_cluster_max_multiplicity", &lsv_cluster_max_multiplicity);
  odtree->SetBranchAddress("pps_counter", &pps_counter);
  odtree->SetBranchAddress("gps_fine_time_counter", &gps_fine_time_counter);
  odtree->SetBranchAddress("lsv_cluster_fixed_width_charge", &lsv_cluster_fixed_width_charge);
  odtree->SetBranchAddress("lsv_n_clusters", &lsv_n_clusters);
  ...
  odtree->SetBranchStatus("event_number",kTRUE);
  odtree->SetBranchStatus("lsv_cluster_max_multiplicity",kTRUE);
  odtree->SetBranchStatus("pps_counter",kTRUE);
  odtree->SetBranchStatus("gps_fine_time_counter",kTRUE);  
  odtree->SetBranchStatus("lsv_cluster_fixed_width_charge",kTRUE);
  odtree->SetBranchStatus("lsv_n_clusters",kTRUE);
...
#define single_run
 ...
 //===============Loop Over Veto Events=========================================    
		...
		for(int ncluster=0; ncluster<lsv_n_clusters; ncluster++)
		  { double od_time = ((double)pps_counter+((double)gps_fine_time_counter)*20.e-9)*1.e+9 + lsv_cluster_start_ns->At(ncluster);
		    ...
		    */
			...
			nv_prompt_ene_hist->Fill(lsv_cluster_fixed_width_charge->At(ncluster));
			nv_prompt_gps_ene_hist->Fill(gps_diff_temp,lsv_cluster_fixed_width_charge->At(ncluster));
			nv_prompt_ntuple->Fill(gps_diff_temp,lsv_cluster_fixed_width_charge->At(ncluster));			
		      }
		    if(gps_diff_temp>middle_time && gps_diff_temp<delay_time)
		      {
			nv_delay_gps_hist->Fill(gps_diff_temp);
			nv_delay_ene_hist->Fill(lsv_cluster_fixed_width_charge->At(ncluster));
			nv_delay_gps_ene_hist->Fill(gps_diff_temp,lsv_cluster_fixed_width_charge->At(ncluster));
			nv_delay_ntuple->Fill(gps_diff_temp,lsv_cluster_fixed_width_charge->At(ncluster));			
		      }
		    if(gps_diff_temp>prompt_time && gps_diff_temp<delay_time)
		      {
			nv_coincidence_gps_hist->Fill(gps_diff_temp);
			nv_coincidence_ene_hist->Fill(lsv_cluster_fixed_width_charge->At(ncluster));
			nv_coincidence_gps_ene_hist->Fill(gps_diff_temp,lsv_cluster_fixed_width_charge->At(ncluster));
			nv_coincidence_ntuple->Fill(gps_diff_temp,lsv_cluster_fixed_width_charge->At(ncluster));	    
		      }		    
		    if(gps_diff_temp<first_time && gps_diff_temp>middle_time)
		      {
			nv_first_gps_hist->Fill(gps_diff_temp);
			nv_first_ene_hist->Fill(lsv_cluster_fixed_width_charge->At(ncluster));
			nv_first_gps_ene_hist->Fill(gps_diff_temp,lsv_cluster_fixed_width_charge->At(ncluster));
			nv_first_ntuple->Fill(gps_diff_temp,lsv_cluster_fixed_width_charge->At(ncluster));			
		      }
  ...
  tpc_gamma_gps_ene_hist->SetDrawOption("colz");
  nv_gps_ene_hist->SetDrawOption("colz");
  nv_coincidence_gps_ene_hist->SetDrawOption("colz");
  nv_prompt_gps_ene_hist->SetDrawOption("colz");
  nv_delay_gps_ene_hist->SetDrawOption("colz");
  nv_first_gps_ene_hist->SetDrawOption("colz");
